...
def aggregate_and_save_fixtures(season_start_year: str) -> None:
    ...
    for gameweek in range(1, GAMEWEEK_COUNT + 1):
        ...
        season_fixtures_df = pd.concat(
            [season_fixtures_df, gameweek_fixtures_df], ignore_index=True
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Retrieve and save fixtures as a csv file for a given English Premier League season"
    )
    parser.add_argument(
        "season_start_year",
        type=str,
        help="The start year of the season (e.g., for the 2023/24 season it's '2023').",
    )
